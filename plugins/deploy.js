...
cmd({
  pattern: 'deploy',
  desc: 'Deploy your own WhatsApp session (Jadibot-style) using a HAIKO~MDX~ session ID from MEGA.nz',
  category: 'tools',
  react: '🤖',
  filename: __filename
}, async (conn, m, { text }) => {
  ...
  const match = text.trim().match(/^HAIKO~MDX~([a-zA-Z0-9_-]+)#([a-zA-Z0-9_-]+)$/);
  if (!match) {
